...
<script type="text/javascript">
	function nationSelect(){
		...
		var nationValue=document.getElementById("nationValue").value;
		for(var i=0;i<nation.options.length;i++){
			if(nation.options[i].value==nationValue){
...
<form method="post" action="${pageContext.request.contextPath }/admin/movieInfoUpdate.do">
	...
	>${vo.genreName }</label></c:forEach><br>
	소개<textarea rows="10" cols="100%" id="movieIntro" name="movieIntro">${vo.movieIntro }</textarea><br/>
	개봉일자<input type="text" id="movieReleaseDate" name="movieReleaseDate" value="${vo.movieReleaseDate }"/><br/>
	...
	<select name="movieAge">
		...
		>전체 관람가</option>
		<option value="1"
			<c:if test="${vo.movieAge=='1' }">selected="selected"</c:if>
		>12세 관람가</option>
		<option value="2"
			<c:if test="${vo.movieAge=='2' }">selected="selected"</c:if>
		>15세 관람가</option>
		<option value="3"
			<c:if test="${vo.movieAge=='3' }">selected="selected"</c:if>
		>청소년관람불가</option>
		<option value="4"
			<c:if test="${vo.movieAge=='4' }">selected="selected"</c:if>
		>제한상영가</option>
	</select></label><br>
